...
function Footer(): JSX.Element {
  ...
  return (
    <div className="w-full h-32 grid grid-template-rows-6 items-center justify-center fixed bottom-0 text-gray-500 border-t-gray-100 border-2 p-2">
        ...
        <ul className="flex items-center justify-center">
          {footerPoliciesLinks.map((link, index) => (
            <li key={link.text} className={`${index === 1 ? 'border-x-2 border-gray-600' : ''} px-3`}>
              <Link href={link.href} className='text-sm font-bold cursor-pointer'>{link.text}</Link>
            </li>
          ))}
        </ul>
      </div>
